<div class="card animate-fade-in">
  <div class="card-header">
    <span class="card-title">Viết bài mới</span>
  </div>
  <div class="card-body">
    <form action="<?= url('/admin/articles/create') ?>" method="POST" enctype="multipart/form-data" id="form-article">
      <?= csrf_field() ?>
      ...
      <div class="flex" style="gap: 24px; flex-wrap: wrap;">
        <div style="flex: 1; min-width: 300px;">
          <div class="form-group">
            ...
            <input type="text" name="title" id="title" class="form-control <?= has_error('title') ? 'is-invalid' : '' ?>" value="<?= old('title') ?>" required>
            <?= error('title') ?>
          </div>
          ...
          <div class="form-group">
            ...
            <textarea name="content" id="content" class="form-control <?= has_error('content') ? 'is-invalid' : '' ?>" rows="15" required><?= old('content') ?></textarea>
            <?= error('content') ?>
            <div class="text-xs text-muted mt-1">Hỗ trợ nhập văn bản có định dạng cơ bản.</div>
          </div>
        </div>
        ...
        <div style="width: 300px;">
          ...
          <div class="form-group">
            ...
            <div class="mt-2" id="image-preview" style="display: none; width: 100%; height: 150px; border-radius: 6px; background-size: cover; background-position: center; border: 1px dashed #2f3349;"></div>
          </div>

          <div class="mt-6">
            <button type="submit" class="btn btn-primary w-full"><i class="fa-solid fa-save"></i> Đăng bài viết</button>
