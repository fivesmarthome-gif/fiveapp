<div class="card animate-fade-in" style="max-width: 800px; margin: 0 auto;">
  <div class="card-header">
    <span class="card-title">Chỉnh sửa khuyến mãi</span>
  </div>
  <div class="card-body">
    <form action="<?= url("/admin/promotions/{$promotion->id}/edit") ?>" method="POST" enctype="multipart/form-data" id="form-promo">
      <?= csrf_field() ?>
      ...
      <div class="flex" style="gap: 20px; flex-wrap: wrap;">
        <div style="flex: 1; min-width: 300px;">
          <div class="form-group">
            ...
            <input type="text" name="name" id="name" class="form-control <?= has_error('name') ? 'is-invalid' : '' ?>" value="<?= old('name', $promotion->name) ?>" required>
            <?= error('name') ?>
          </div>
          ...
          <div class="flex" style="gap: 16px;">
            ...
            <div class="form-group" style="flex: 1;">
              ...
              <input type="number" name="discount_value" id="discount_value" class="form-control <?= has_error('discount_value') ? 'is-invalid' : '' ?>" value="<?= old('discount_value', $promotion->discount_value) ?>" required min="0">
        ...
        <div style="flex: 1; min-width: 300px;">
          <div class="form-group">
            ...
            <input type="text" name="code" id="code" class="form-control <?= has_error('code') ? 'is-invalid' : '' ?>" value="<?= old('code', $promotion->code) ?>" required style="text-transform: uppercase;">
          ...
          <div class="flex" style="gap: 16px;">
            <div class="form-group" style="flex: 1;">
              ...
              <input type="datetime-local" name="start_date" id="start_date" class="form-control <?= has_error('start_date') ? 'is-invalid' : '' ?>" value="<?= old('start_date', date('Y-m-d\TH:i', strtotime($promotion->start_date))) ?>" required>
              <?= error('start_date') ?>
            </div>
            
            <div class="form-group" style="flex: 1;">
              <label class="form-label" for="end_date">Ngày kết thúc<span class="required">*</span></label>
              <input type="datetime-local" name="end_date" id="end_date" class="form-control <?= has_error('end_date') ? 'is-invalid' : '' ?>" value="<?= old('end_date', date('Y-m-d\TH:i', strtotime($promotion->end_date))) ?>" required>
              <?= error('end_date') ?>
            </div>
          </div>
          ...
          <div class="form-group">
            ...
            <?php $hasImage = !empty($promotion->image); ?>
            <div class="mt-2" id="image-preview" style="display: <?= $hasImage ? 'block' : 'none' ?>; width: 100%; height: 120px; border-radius: 6px; background-size: cover; background-position: center; border: 1px dashed #2f3349; <?= $hasImage ? 'background-image: url(' . url('/' . ltrim($promotion->image, '/')) . ')' : '' ?>"></div>
          </div>
        </div>
      </div>
      ...
      <div class="flex justify-end gap-3 mt-6">
        ...
        <button type="submit" class="btn btn-primary"><i class="fa-solid fa-save"></i> Cập nhật khuyến mãi</button>
      </div>
    </form>
  </div>
</div>
