<div class="card animate-fade-in" style="max-width: 600px; margin: 0 auto;">
  <div class="card-header">
    <span class="card-title">Thêm loại sản phẩm mới</span>
  </div>
  <div class="card-body">
    <form action="<?= url('/admin/product-types/create') ?>" method="POST" id="form-product-type">
      <?= csrf_field() ?>
      ...
      <div class="form-group">
        ...
        <select name="category" id="category" class="form-control">
          <option value="Răng Sứ" <?= old('category') == 'Răng Sứ' ? 'selected' : '' ?>>Răng Sứ (Cố định)</option>
          <option value="Tháo Lắp" <?= old('category') == 'Tháo Lắp' ? 'selected' : '' ?>>Hàm Tháo Lắp</option>
          <option value="Khác" <?= old('category') == 'Khác' ? 'selected' : '' ?>>Loại khác</option>
        </select>
      </div>
      ...
      <div class="flex" style="gap: 16px;">
        <div class="form-group" style="flex: 1;">
          ...
          <input type="number" name="base_price" id="base_price" class="form-control <?= has_error('base_price') ? 'is-invalid' : '' ?>" value="<?= old('base_price', '0') ?>" required min="0" step="1000">
          <?= error('base_price') ?>
        </div>

        <div class="form-group" style="flex: 1;">
          <label class="form-label" for="estimated_days">Thời gian hoàn thành (Ngày)</label>
          <input type="number" name="estimated_days" id="estimated_days" class="form-control" value="<?= old('estimated_days', '5') ?>" min="1">
        </div>
      </div>
      ...
      <div class="flex justify-end gap-3 mt-6">
        ...
        <button type="submit" class="btn btn-primary"><i class="fa-solid fa-save"></i> Thêm sản phẩm</button>
      </div>
    </form>
  </div>
</div>
