<div class="card animate-fade-in" style="max-width: 600px; margin: 0 auto;">
  <div class="card-header">
    <span class="card-title">Chỉnh sửa chi nhánh</span>
  </div>
  <div class="card-body">
    <form action="<?= url("/admin/branches/{$branch->id}/edit") ?>" method="POST" id="form-branch">
      <?= csrf_field() ?>

      <div class="form-group">
        <label class="form-label" for="name">Tên chi nhánh<span class="required">*</span></label>
        <input type="text" name="name" id="name" class="form-control <?= has_error('name') ? 'is-invalid' : '' ?>" value="<?= old('name', $branch->name) ?>" required>
        <?= error('name') ?>
      </div>
      ...
      <div class="form-group">
        ...
        <input type="text" name="phone" id="phone" class="form-control <?= has_error('phone') ? 'is-invalid' : '' ?>" value="<?= old('phone', $branch->phone) ?>" required>
        <?= error('phone') ?>
      </div>

      <div class="form-group">
        <label class="form-label" for="hotline">Hotline hỗ trợ</label>
        <input type="text" name="hotline" id="hotline" class="form-control" value="<?= old('hotline', $branch->hotline) ?>">
      </div>
      ...
      <div class="flex justify-end gap-3 mt-6">
        ...
        <button type="submit" class="btn btn-primary"><i class="fa-solid fa-save"></i> Cập nhật chi nhánh</button>
      </div>
    </form>
  </div>
</div>
